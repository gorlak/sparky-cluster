"""The runbook trigger (ADR-0021, ADR-0011 Layer 3) — no systemd, no deploy.

`vllm-runbook` is a *grant*: it runs as root, and anything that can reach the control
...
TRIGGER = (Path(__file__).resolve().parent.parent
           / "ansible/roles/activate/files/vllm-runbook")
...
def _load():
    ...
    spec = importlib.util.spec_from_loader(
        "vllm_runbook",
        importlib.machinery.SourceFileLoader("vllm_runbook", str(TRIGGER)),
    )
...
def test_a_name_with_shell_metacharacters_is_refused(installed):
    ...
    only be interesting to a shell is a caller probing for one, and it is not a runbook."""
...
def test_an_installed_runbook_is_accepted(installed):
...
def test_the_client_and_the_trigger_agree_on_the_paths():
    ...
    assert runbookctl.UNIT == trigger.UNIT
    assert runbookctl.LOG_DIR == trigger.LOG_DIR
    assert runbookctl.INSTALLED_DIR == trigger.RUNBOOK_DIR
    assert runbookctl.log_path("x") == trigger.LOG_DIR / "x.log"


def test_the_unit_runs_the_foreground_runner_not_the_launcher():
    """`run` is the launcher that reaches this trigger; `sweep` is what actually runs. If
    the unit executed `run`, starting a runbook would start a launcher that starts a
    launcher."""
