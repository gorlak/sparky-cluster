#!/usr/bin/env bash
# =============================================================================
# snoopy step 2: install model from pre-staged copy
# =============================================================================
# The model weights were downloaded on the Mac (~235GB,
# Qwen/Qwen3-235B-A22B-Instruct-2507-FP8) and pushed to this node via
# `push-model.sh snoopy`. They currently live at ~/model-cache/... under the
# invoking user's home. This step moves them into /opt/vllm/models/ and
# transfers ownership to the `vllm` service user.
#
...
#
# Usage (from the Mac):
#   ssh snoopy 'sudo bash ~/Projects/DGX-Spark-Setup/nodes/snoopy/scripts/install-step2.sh'
# =============================================================================
...
MODEL_NAME="${1:-Step-3.5-Flash-FP8}"
...
if [[ -f "$FINAL_DIR/config.json" ]]; then
    ...
    [[ -f "$STAGING_DIR/config.json" ]] \
        || die "staged model missing or incomplete — rsync from sparky first"
...
log "snoopy step 2 complete."
log "ensure sparky's step 2 has also completed before running sparky's step 5."
