"""Suites — named, reviewable procedures (ADR-0020).

A suite is to a *procedure* what a profile is to a *model*: a declarative artifact in
the repo, installed by `deploy`, instanced by name. `./sparky.sh run <name>`.

**The safety property, and the reason it is derived rather than written down.** A suite
...
Anything that can instance a suite runs it as `activator` — an identity holding two
...
The runner itself is `suite.py`. This module owns naming, discovery and validation — what
...
#
# The REPO is where a suite is authored and reviewed, in a diff, like `ansible/profiles/`.
...
# be in a git checkout, and the moment two callers consult different lists, "which suites
...
class UnknownSuite(LookupError):
    """No such suite. Raised with the list, because a typo and a missing deploy look
    identical from the outside and the fix differs."""


class BadSuite(ValueError):
...
def describe(directory: Path | None = None) -> list[dict]:
    ...
    For the surfaces that OFFER a suite rather than run one. A bare name is a poor
    ...
    for name in names_in(directory):
        ...
        except (UnknownSuite, BadSuite, Exception):  # noqa: B014 - any bad file
            spec = {}
        out.append({
            "name": name,
    ...
    # forget when a suite is added, and it would live nowhere in particular.
    return sorted(out, key=lambda r: (r["order"], r["name"]))


# Unordered suites land between the deliberate ones and nothing in particular, which is
...
def path_for(name: str, *, directory: Path | None = None) -> Path:
    ...
    if "/" in name or name.startswith("."):
        raise BadSuite(f"{name!r} is a name, not a path")
...
def _coverage_problems(name: str, spec: dict, jobs: list) -> list[str]:
    ...
    that is drift — add a profile, forget the suite, and the gap shows up much later as
    ...
    find before a deploy rather than after a seven-hour suite.
    """
...
def validate(name: str, spec: dict | None = None, *,
    ...
    """Problems with a suite, as a list. Empty means it is runnable.
    ...
    Returns rather than raises so `lint` can report every fault in every suite at once —
            ...
            if isinstance(step, dict) and "cmd" in step:
                ...
                if not isinstance(step.get("args", []), list):
                    problems.append(
                        ...
                        f"string would be a shell command, which a suite may never be.")
    ...
    # that starts a suite is in a web page, and whoever presses it is not reading this
    ...
    if not str(spec.get("description") or "").strip():
        problems.append(f"{name}: no `description:` — it labels the panel button and "
                        f"`sparky run`, and a multi-hour suite has to say what it does "
