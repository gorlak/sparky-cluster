"""Start, stop and read a detached suite run (ADR-0021).

The client half of `/usr/local/sbin/vllm-suite`, and deliberately thin — every decision
about *what may run* lives in the trigger, which is the copy a network caller also reaches.
Duplicating the allowlist check here would produce a second answer to "which suites
...
# constants. tests/test_suitectl.py asserts they do — a drift here would fail at the
# worst moment, with a run that starts and a log nobody can find.
SUITE_BIN = "/usr/local/sbin/vllm-suite"
...
def running() -> str | None:
    """The suite currently running, or None.
    ...
    if fields.get("ActiveState") not in ("active", "activating", "deactivating"):
        return None
    return fields.get("Description", "").removeprefix("sparky suite: ") or UNIT
...
def last_exit() -> tuple[str, int | None]:
    ...
    last night's suite finish clean?" is most of why any of this is logged.
    """
    out = subprocess.run(
...
def start(name: str) -> int:
    ...
    return subprocess.run([*_sudo(), SUITE_BIN, "start", name]).returncode


def stop() -> int:
    return subprocess.run([*_sudo(), SUITE_BIN, "stop"]).returncode
...
def follow(name: str, *, once: bool = False) -> int:
    ...
    whole history of that suite's runs — a suite resumes, so its runs are episodes of
    one suite and reading them together is the point.
    """
    path = log_path(name)
    if not path.exists():
