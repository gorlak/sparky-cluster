...
6. **The `all` suite** — add the profile to `suites/all.yml`. It declares
...
A suite is a named, reviewable procedure (ADR-0020) — `suites/<name>.yml`, started by
...
1. **Write `suites/<name>.yml`.** Jobs are `{profile, regiments}`; `defaults.regiments`
...
2. **Put the decision rule in the file, before the numbers exist.** A suite that
   ...
   **`covers: allowlist`** instead, and `lint` then fails whenever the two disagree.
   `suites/all.yml` is the one that does.
3. **`./sparky.sh lint`** validates the repo's suites alongside profiles — a step naming
   a privileged command fails here rather than two hours into a run.
4. **Try it in the foreground first**: `./sparky.sh suite suites/<name>.yml --dry-run`
   prints the plan and runs nothing; without `--dry-run` it runs the job list from its
   path, which is the iteration loop before the suite is installed.
5. **`./sparky.sh deploy`** publishes it to `/opt/cluster/suites/`. Until then
