...
| **DEF-0003** | GB10 cudagraph inference hang (`FULL_AND_PIECEWISE`) | 26.06 profiles once serving **that do not use speculative decoding** | [vllm#40969](https://github.com/vllm-project/vllm/issues/40969) | **none deliberate — masked by accident.** No profile sets `cudagraph_mode`/`--enforce-eager`; both live 26.06 profiles use MTP spec-decode, and vLLM auto-downgrades `FULL_AND_PIECEWISE`→`PIECEWISE` for spec-decode on FlashInfer. Remove spec-decode and the mask goes with it | #40969 fixed — **re-test on a 26.06 profile WITHOUT spec-decode** (the only shape that actually exercises full graphs), then A/B throughput (ADR-0014) | 🟡 WAR'd (incidentally) | [26.06 tracker](upgrades/container-nvidia-vllm-26.06-py3.md) · [ADR-0014](adr/0014-optimization-register.md) |
