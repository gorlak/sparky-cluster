...
| **DEF-0006** | Step-3.7 `Step3VLProcessor` crash-loops on startup — `AttributeError: … '_get_num_multimodal_tokens'` (VL-processor, orthogonal to NVFP4/container) | `step-3.7-nvfp4` profile (blocked, hidden from deploy UI) | **unfiled** — vLLM `Step3VLProcessor` / `transformers/multimodal.py` | none — profile shelved until upstream fix | vLLM ships a `Step3VLProcessor` with `_get_num_multimodal_tokens` — **re-test** the `step-3.7-nvfp4` deploy | 🔴 open | [profile-step-3.7 tracker](upgrades/profile-step-3.7-flash.md) |
