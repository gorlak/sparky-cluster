...
| **DEF-0002** | TP=2 inference-time CUDA deadlock, 35–55 min into serving (2× Spark) | sustained 26.06 TP=2 serving | [vllm#41725](https://github.com/vllm-project/vllm/issues/41725) | none confirmed — PP=2 sidesteps (pipeline-bubble latency) | #41725 fixed/closed — **soak-test** 26.06 TP=2 for hours (this is the real gate for sustained serving) | 🔴 open | [26.06 tracker](upgrades/container-nvidia-vllm-26.06-py3.md) |
...
| **DEF-0006** | Step-3.7 `Step3VLProcessor` crash-loops on startup — `AttributeError: … '_get_num_multimodal_tokens'` (VL-processor, orthogonal to NVFP4/container) | `step-3.7-nvfp4` profile (blocked, hidden from deploy UI) | **unfiled** — vLLM `Step3VLProcessor` / `transformers/multimodal.py` | **unblock path found (2026-07 sweep):** StepFun ships a prebuilt image `vllm/vllm-openai:stepfun37` + an official vLLM Step-3.7 recipe — buildable/pullable via the `images` role (ADR-0013) | deploy on a container carrying the fixed processor (StepFun `stepfun37` image, or a stock vLLM once the method lands) — **re-test** `step-3.7-nvfp4` | 🟡 WAR'd (container path exists) | [profile-step-3.7 tracker](upgrades/profile-step-3.7-flash.md) |
