...
**Status:** 🔵 Evaluated — not started
**Profile:** `step` (currently serves `Step-3.5-Flash-FP8`, TP=2, gmu 0.90)
**Target:** Step-3.7-Flash — FP8 runs on the current container; **NVFP4 is the goal**,
gated on the container upgrade
...
- **NVFP4 path (3.7-NVFP4):** the compelling one — it *changes the profile's character*,
  ...
  the same "big-shared with headroom" class as the `minimax` profile.
...
`ansible/profiles/step.yml` at it and `make deploy PROFILE=step`.
