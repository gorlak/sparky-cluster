# ADR-0021: Suite runs get a detached, logged home — the panel starts them
...
ADR-0020 made a suite a named, reviewable artifact started by `./sparky.sh run <name>`.
What it did not give it is somewhere to *live*. A suite run is hours long, and today it
...
suite run needs a detached and logged environment. Nothing more.** Not a scheduler, not
...
**The constraint is lifetime, not privilege.** A suite activates — already granted to
...
**A suite run is always a transient systemd unit, started through one fixed program —
`/usr/local/sbin/vllm-suite` — and appending its output to a per-suite log.**
...
`POST /suite/{name}` on the panel does the same thing. Neither is a special case, and
...
for a job list that is not (yet) an installed suite.

Bounded exactly like `vllm-activate` and `vllm-probe`:

- **The name is checked against an installed allowlist** — `/opt/cluster/suites/`,
  ...
  Adding a suite is therefore a deploy — the same bargain profiles already make, and the
...
fails within minutes, while a suite holds the whole fleet for hours and the operator may
...
- **Adding a suite becomes a deploy**, and `sparky run` no longer runs anything from the
...
- **Anyone who can reach `/admin` can commandeer the cluster for hours.** It is basic_auth'd
  ...
  kind — but an unwanted activation costs minutes and an unwanted suite costs an evening.
...
running as an identity with sudoers entries. Naming suites was what made the invocable
