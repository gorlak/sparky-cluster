# ADR-0020: Runbooks — named, reviewable procedures (`sparky run <name>`)

**Date:** 2026-08-10
**Status:** Accepted (implemented 2026-08-11)
**Errata 2026-08-11:** [ADR-0021](0021-runbook-runs-from-the-panel.md) revised two things
...
ADR-0019's probe bounded — "probing something new is a deploy". Runbooks are the third
instance of that pattern and currently the only one done by hand.

## Decision

**A runbook is a declarative artifact, deployed like a profile, instanced by name.**

```
runbooks/<name>.yml        in the repo — the allowlist, reviewed in a diff
...
install runbooks to `/opt/cluster/runbooks/` on the profile pattern. Nothing needs it:
...
> callers rather than one: `/opt/cluster/runbooks/` is now the installed allowlist and
...
A runbook step names a `sparky` subcommand and its arguments as a list:
...
1. **A runbook cannot provision.** `deploy` is out of scope, and would fail anyway — it
   needs a TTY for the sudo password that no unattended runner has.
2. **The vocabulary tracks the tool.** A new Operate command is usable by runbooks the
...
`activator`, an identity holding two single-command sudoers entries. A runbook that could
...
The obvious next step is `POST /runbooks/<name>/run` on the control panel, so a multi-hour
...
> **That ADR is [0021](0021-runbook-runs-from-the-panel.md) (2026-08-11).** It landed the
> day after this one, and answered the question differently than framed: the constraint
> turned out to be *lifetime*, not authorization — a runbook run needs no privilege the
...
- **A procedure becomes reviewable.** A runbook is read and approved before it takes the
  cluster for two hours, in a diff, rather than being typed at 1am and patched at 2am.
- **`docs/updating.md` gains a pathway.** Adding a runbook is a change like any other and
...
- **Results land where they already land.** Regiments record to the trend store, and
  ...
  recorded measurement — so a runbook updates the scoreboard with no new plumbing. That
  pipeline exists today; this ADR only gives the front of it a name.
- **`sweeps/` becomes `runbooks/`.** A sweep is one *kind* of runbook, and naming the
...
**A `--detach` flag plus a runbook page in the docs.** Cheaper, and it *was* the right
...
drift — with the failure mode being a runbook that silently does nothing.

**Arbitrary shell in a runbook step.** Rejected on the grounds above: it converts anything
that can instance a runbook into a remote shell running as an identity with sudoers
