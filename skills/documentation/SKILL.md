...
| `docs/models/retired/<model>.md` | **Retirement report** | A retired model or shape: the reasoning **and** the profile YAML, embedded in a fenced block. The config is kept because reviving something costs a deploy and the config is the expensive part to reconstruct — parser names, flag combinations, memory math. It is **not** kept as a live-looking `.yml`: a file nothing parses, sitting in the directory that *is* the allowlist, invites "why isn't this running?" and answers it only if you read to the end. The report is **not maintained** — its status and next-step sections are history, not a plan, and the verdict is never here (that is the tombstone register). **Do not read one unless its filename is the model you are actually investigating**: a 250-line sheet saying "Target quant: NVFP4 — preferred" outweighs a one-line tombstone row in anyone's attention, which is how a rejected model gets re-proposed. `variant-shapes.md` covers retired `-single`/`-mtp3` variants, whose models are mostly still in the fleet. |
